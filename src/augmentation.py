...
def augment_image(img, steering_angle, training_model=True):
    ...
    if random.random() < 0.3:
        ...
        factor = 0.7 + np.random.uniform() * 0.6  # range from 0.7 to 1.3
    ...
    if random.random() < 0.3:
        ...
        # left right shift
        x_shift = int((np.random.rand() - 0.5) * 0.1 * w)
        # top bottom shift
        y_shift = int((np.random.rand() - 0.5) * 0.1 * h)
